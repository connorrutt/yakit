...
const UILayout: React.FC<UILayoutProp> = (props) => {
    ...
    useEffect(() => {
        ipcRenderer.on("update-yaklang-reconnect-callback", () => updateReconnect())
        ...
        <div className={styles["ui-layout-wrapper"]}>
            ...
            {isYakInstalled ? (
                <div className={styles["ui-layout-body"]}>
                    ...
                    {isRemoteEngine && (
                        <RemoteYaklangEngine onSubmit={onSubmitRemoteLink} onCancel={() => onCancelRemoteLink(true)} />
                    )}
                </div>
            ) : (
                <div className={styles["ui-layout-mask"]}>
                    {isRemoteEngine ? (
                        <RemoteYaklangEngine onSubmit={onSubmitRemoteLink} onCancel={() => onCancelRemoteLink(false)} />
                    ) : (
                        <YaklangEngineHint
                            system={system}
                            visible={engineShow}
...
interface YaklangEngineHintProps {
    ...
    startEngine: (sudo: boolean, port?: number) => any
}
...
const DownloadYakit: React.FC<DownloadYakitProps> = React.memo((props) => {
    ...
    const [disabled, setDisabled] = useState(false)
    const [bounds, setBounds] = useState({left: 0, top: 0, bottom: 0, right: 0})
    const debouncedBounds = useDebounce(bounds, {wait: 200})
    const draggleRef = useRef<HTMLDivElement>(null)
...
const YakitQuestionModal: React.FC<AgrAndQSModalProps> = React.memo((props) => {
    ...
    const [disabled, setDisabled] = useState(false)
    const debouncedDisable = useDebounce(disabled, {wait: 500})
    const [bounds, setBounds] = useState({left: 0, top: 0, bottom: 0, right: 0})
    const draggleRef = useRef<HTMLDivElement>(null)
